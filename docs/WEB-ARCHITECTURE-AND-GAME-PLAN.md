...
| 7.3 | **Strength trend on Progress.** Per-lift best-e1RM-per-week line from loaded logs (8-week window; lazy-fetch older). | `ProgressScreen.jsx`, `insightsBuilders.js` | **Done.** `exercise_name` now rides the log window; `buildStrengthTrends` (best e1RM per lift per week, grouped by the recorded max's key, ≥2 weeks) → `StrengthTrends` sparklines + "+N lbs this block". Only lifts with a recorded max are tracked. |
| 7.4 | **MCP parity.** `generate_weekly_summary` + context bundle include e1RM PRs so the coach celebrates the same moments. | `mcp/src/tools/queries.ts`, contract tests | **Done.** `mcp/src/e1rm.ts` (`findEstimatedPrs`, best set vs `current_user_maxes`, keyed by max name). `generate_weekly_summary` returns `estimated_prs` and adds "Estimated PRs (not saved yet)" to the recap; skips lifts already saved this week. Test in `mcp/src/__tests__/e1rm.test.ts`. Context bundle unchanged (recap is where the coach celebrates). |
