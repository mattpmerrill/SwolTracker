...
- 2026-04-24: **Prod hotfix** — cron-triggered `swoltracker-daily-nudge` was returning HTTP 405 on all `/api/mcp` calls since 2026-04-22. Root cause: Phase 4.3 made `buildApp()` read `SKILL.md` at boot via `loadSkill(resolve(root ?? process.cwd(), skillFile))`, but `vercel.json` only included the file for the legacy `api/mcp.js` function — and only as an exact literal, so the new `api/mcp/*.js` subroutes (openapi, context, skill) shipped without it. Fix: `vercel.json` now lists `"includeFiles": "SKILL.md"` on `api/mcp.js` and `api/mcp/openapi.js`. Commit `1d14dde`. Gotcha recorded in CLAUDE.md "Operational gotchas" section.
- 2026-04-24: **Code-review pass** — full review of SwolTracker + vendored `@bot-native/sdk` looking for bugs, not features. Three fixes landed (commit `79c25d2`):
  - `fuzzyMatch` in `mcp/src/tools/natural-language.ts` now throws `AppError.invalidArgs` when multiple program exercises tie at the winning match tier (exact / substring / word-overlap) instead of silently picking the first. Callers at `log_exercise` (line ~177) and `log_workout_summary` (line ~295) propagate the throw.
  - `repsSchema` in `mcp/src/sdk-adapter.ts:40` requires `reps >= 1` (was `>= 0`). Zero-rep sets were never meaningful logs.
  - `bulk_log_workout` in `mcp/src/tools/natural-language.ts` now throws on non-finite or out-of-range `weight_lbs` (< 0 or > 9999) instead of silently dropping the field, matching the surrounding error-on-bad-input pattern.
  - Build + 154 vitest tests green. Deployed via GitHub integration.
  - **Deferred findings** (tracked here so we don't lose them): (1) `update_profile` and `complete_onboarding` in `sdk-adapter.ts` currently require `write:logs` — that scope name is wrong for profile writes; probably wants a new `write:profile` scope or to be folded into `write:program`. Non-urgent because all existing keys carry the full scope set (migration 030 backfill). (3) `server.js:18-24` in the SDK (`toolResultFromThrown`) leaks `error.message` verbatim to agents for non-`AppError` throws — should sanitize unknown errors before surfacing. Deferred — low blast radius given typed-error coverage in our tool bodies.
